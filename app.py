...
class Jarvis():
    ...
    def run_jarvis(self):
        ...
        while True:
            ...
            print(self.command)
            if ('play a song' in self.command) or ('youtube' in self.command) or (
                    "download a song" in self.command) or ("download song" in self.command):
            ...
            elif ('weather forcast' in  self.command) or ('what is current weather status' in self.command):
                self.weather()

            # Interaction commands with JARVIS
            elif ('your age' in self.command) or ('are you single' in self.command) or (
                    'are you there' in self.command) or ('tell me something' in self.command) or (
                    'thank you' in self.command) or ('in your free time' in self.command) or (
                    'i love you' in self.command) or ('can you hear me' in self.command) or (
                    'do you ever get tired' in self.command):
            ...
            elif (('hi' in self.command) and len(self.command) == 2) or (
                    (('hai' in self.command) or ('hey' in self.command)) and len(self.command) == 3) or (
                    ('hello' in self.command) and len(self.command) == 5):
                self.comum(self.command)


            elif ('what can you do' in self.command) or ('your name' in self.command) or (
                    'my name' in self.command) or ('university name' in self.command):
                self.Fun(self.command)
            ...
            elif ('silence' in self.command) or ('silent' in self.command) or ('keep quiet' in self.command) or (
                    'wait for' in self.command):
            ...
            elif ('facebook' in self.command) or ('whatsapp' in self.command) or ('instagram' in self.command) or (
                    'twitter' in self.command) or ('discord' in self.command) or ('social media' in self.command):
            ...
            elif ('open teams' in self.command) or ('open stream' in self.command) or (
                    'open sharepoint' in self.command) or ('open outlook' in self.command) or (
                    'open erp portal' in self.command) or ('open aims' in self.command):
            ...
            elif ('wikipedia' in self.command) or ('what is meant by' in self.command) or (
                    'tell me about' in self.command) or ('who the heck is' in self.command):
            ...
            elif ('open gmail' in self.command) or ('open maps' in self.command) or (
                    'open calender' in self.command) or ('open documents' in self.command) or (
                    'open spredsheet' in self.command) or ('open images' in self.command) or (
                    'open drive' in self.command) or ('open news' in self.command):
            ...
            elif ('open calculator' in self.command) or ('open notepad' in self.command) or (
                    'open paint' in self.command) or ('open online classes' in self.command) or (
                    'open discord' in self.command) or ('open ltspice' in self.command) or (
                    'open editor' in self.command) or ('open spotify' in self.command) or (
                    'open steam' in self.command) or ('open media player' in self.command):
            ...
            elif ('close calculator' in self.command) or ('close notepad' in self.command) or (
                    'close paint' in self.command) or ('close discord' in self.command) or (
                    'close ltspice' in self.command) or ('close editor' in self.command) or (
                    'close spotify' in self.command) or ('close steam' in self.command) or (
                    'close media player' in self.command):
            ...
            elif ('take screenshot' in self.command) or ('screenshot' in self.command) or (
                    "take a screenshot" in self.command):
                self.scshot()
            ...
            elif ("volume up" in self.command) or ("increase volume" in self.command) or ("increase more" in self.command):
                pyautogui.press("volumeup")
                self.talk('volume increased')

            # command for decreaseing the volume in the system
            # Eg: jarvis decrease volume
            elif ("volume down" in self.command) or ("decrease volume" in self.command) or ("decrease more" in self.command):
            ...
            # Individual => Eg: send a message to Mahaveer
            ...
            elif ("you can sleep" in self.command) or ("sleep now" in self.command) or ("you can take rest now" in self.command):
            ...
            elif 'can u search on chatbot' in self.command:
                self.chatbot()
    ...
    def temperature(self):
        geo_data = get('https://ipinfo.io/json').json()
        city = geo_data['city']
    ...
    def Fun(self, command):
        ...
        if 'your name' in command:
            ...
            self.talk("your name is Mahaveer")
        elif 'university name' in command:
            self.talk(
                "you are pursuing your B.Tech from Indian Institute of Technology, Hyderabad")
    ...
    def college(self, command):
        ...
        elif 'erp portal' in command:
            self.talk('opening your IITH  ERP Portal')
            webbrowser.open('https://erp.iith.ac.in/Default/Authenticate.html')
        elif 'aims' in command:
            self.talk('opening IITH aiims portal')
            webbrowser.open('https://aims.iith.ac.in/aims/')
        elif 'college website' in command:
            self.talk('opening IITH main website')
            webbrowser.open('https://www.iith.ac.in/')
        else:
            self.No_result_found()
    ...
    def open_source(self, command):
        ...
        if 'github' in command:
            ...
            webbrowser.open('https://github.com/RegarMonu?tab=repositories')
        else:
            self.No_result_found()
    ...
    def silenceTime(self, command):
        ...
        if ('10' in command) or ('ten' in command):
            x = 600
        elif '1' in command or ('one' in command):
            x = 60
    ...
    def location(self):
        ...
        try:
            ...
            city = get('https://ipinfo.io/json').json()['city']
    ...
    # News
    def news(self):
        MAIN_URL_ = "https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=bfbf820724c345daa958a11e6c8d6cde"
    ...
    def weather_data(self,json_data, city):
        ...
        weather_details = ''
        weather_details + (
                "The weather in {} is currently {} with a temperature of {} degrees celcius and wind speeds reaching {} kilometres per hour".format(
                    city, weather_type, temperature, wind_speed))  # def for weather collection

        return weather_details

    def weather(self):
        api_key = 'c8cc8678190d387694b28465a3b81c91'
        city = get('https://ipinfo.io/json').json()['city']
        final_url = f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid={api_key}&units=metric'  # from openweather.org
        json_data = get(final_url).json()  # obtains final data set.
        weather_details = self.weather_data(json_data, city)
        self.talk(weather_details)

    def chatbot(self):
        try:
            self.talk('what information you want to know')
            query = self.command()
            self.talk('can you wait little bit I am fetching data')
            ...
            self.talk("Sorry, I encountered an error.")
